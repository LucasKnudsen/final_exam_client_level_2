describe('User can see a list of all courses', () => {
  ...
  it('is expected to display the content of the courses', () => {
    cy.get('[data-cy=course]').first().within(() => {
      ...
      cy.get('[data-cy=instructors]').should('contain', 'Thomas Ochman, Emma-Maria Thalén')
      cy.get('[data-cy=info]').should('contain', 'up to 1 day on-site.(2 to 7 hours depending on audience, please contact us for deetails) This course can be delivered as a mix of theory sessions and code-along practical examples or strictly as a demo session.')
      cy.get('[data-cy=price]').should('contain', 'From 1 000 SEK - €100 (Contact us for details)')
